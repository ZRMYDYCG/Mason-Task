...
import { Table, Tag, TableColumnProps, Pagination, Avatar, Empty } from 'antd'
import dayjs from 'dayjs'
import { ProjectItem, TaskItem, LoginUser } from '@/api/type'
import TaskSetting from '@/pages/workbench/components/task-setting.tsx'
import api from '@/api'
  ...
  const columns: TableColumnProps<TaskItem>[] = [
    {
      ...
      dataIndex: 'key',
      align: 'center',
      render: (_, _r, index) => <div>{index + 1}</div>,
    },
    {
      title: '任务名称',
      dataIndex: 'name',
      ...
      render: (_, record) => (
        <div className="flex items-center">
          <div className="flex w-[45%] justify-end" onClick={e => e.stopPropagation()}>
          ...
          <div className="ml-2 flex flex-1 justify-start">{record.name}</div>
        </div>
      ),
    },
    {
